...
export default function DreamParallax() {
    ...
    const createAdvancedGradientMaterial = () => {
      return new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          const float SHININESS = 32.0; // For specular power
          ...
          void main() {
            ...
            rimFactor = pow(rimFactor, 2.0); // Sharpen the rim
            vec3 rimLightColor = vec3(0.8, 0.8, 0.8) * rimFactor * 0.4; // Soft white rim
  ...
  { x: 22, y: 3 }     // M (a bit further right)
    ...
    function animate() {
      ...
      const animationProgress = THREE.MathUtils.smoothstep(scrollProgress, 0.1, 0.5);

      letterData.forEach(({ 
        group, material, index, targetX, targetY, baseZ, 
        randomRotX, randomRotY, randomRotZ
      }) => {
        
        group.position.y = THREE.MathUtils.lerp(-20, targetY, animationProgress);
        group.position.x = THREE.MathUtils.lerp(0, targetX, animationProgress);
        group.position.z = THREE.MathUtils.lerp(-10, baseZ, animationProgress);
        ...
        material.uniforms.uCameraPosition.value.copy(camera.position); // Ensure camera position is updated
      });
  ...
  return (
    ...
    >
      ...
      >
        {/* Text (z-10, peeche) */}
        <div 
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-10 text-center w-3/5 pointer-events-none"
        >
          <h2 className=" text-3xl md:text-4xl lg:text-5xl text-white leading-normal font-['eight']">
            We are a creative and passionate team of corporate event planners 
            delivering exceptional experiences across India. From concept to 
            execution, we ensure every event is innovative, seamless and memorable.
          </h2>
        </div>
